    ...
    stage('Deploy') {
        steps {
            ...
            withCredentials([aws(credentialsId: "EB-Managed-Platform-Update", accessKeyVariable: 'AWS_KEY', secretKeyVariable: 'AWS_SECRET')]) {
                ...
                bat "aws elasticbeanstalk update-environment --application-name=${AWS_EB_APP} --environment-name=${AWS_APP_ENV} --option-settings Namespace=aws:elasticbeanstalk:application:environment,OptionName=PARAM4,Value=Param5Value"
